...
Иными словами: идея метода 'bind' в том, что мы берём какой-то метод и вызываем его в контексте объекта */
...
function bind(fn, context, ...rest) {
  ...
  return function (...args) {
    ...
    //context[uniqId]() - мы знаем что это функция (метод) и вызываем её в контексте объекта person[uniqId] и записываем результат в 'result'
...
//Метод slice в JS используется для извлечения части массива и возврата нового массива без изменения исходного массива. Синтаксис метода среза следующий: array.slice(start, end), где start — это индекс первого элемента, который нужно включить в новый массив, а end — это индекс первого элемента, который нужно исключить из нового массива. Если конец не указан, все элементы от начала до конца исходного массива включаются в новый массив. Call юзаем, чтобы превратить полученные данные из массива (arguments) в новый массив:
...
//4 способ (ES6) - самый эффективный
//function bind(fn, context, ...rest) {
//  return function (args) {
//    return fn.apply(context, rest.concat(args))
//  }
...
//function bind(fn, context, ...rest) {  //2-й вариант (call)
//  return function (...args) {
...
//    const result = context[uniqId](...rest.concat(args)) //вызвали функцию (ktonakh) в контексте объекта и занесли результат в result
